		...
		for odata_type in odata_types:
			...
			'''
 			if 'child' in redfish_data_info.keys():
				...
				attr_properties, attr_navigation_properties, collection_path = get_all_property(os.path.join(RESOURCE_XML_PATH, temp_odata_type+"_v1.xml"), temp_odata_type, collection_path)
				print(attr_properties)
				print(attr_navigation_properties)
				print(collection_path)
				data = create_content(redfish_data_info['child'], collection_path[temp_odata_type][0], attr_properties, attr_navigation_properties)

			'''
			attr_properties, attr_navigation_properties, collection_path = get_all_property(os.path.join(RESOURCE_XML_PATH, odata_type+"_v1.xml"), odata_type, collection_path)
			data = create_content(redfish_data_info, collection_path[odata_type], attr_properties,attr_navigation_properties)
			#dir_path = os.path.join(REDFISH_DATA,odata_type)
